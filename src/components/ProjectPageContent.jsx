...
export default function ProjectPageContent({ project, relatedProjects }) {
      ...
      <section className={`pt-4 pb-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto ${isVisible ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-10'}`}
        ...
        <div className="flex flex-wrap gap-2 mt-6 justify-center">
          {project.tags?.map((tag, index) => (
            <span 
              key={index}
              className={`px-3 py-1 text-sm font-medium ${tag.bg || `bg-${project.primaryColor || 'cyan'}-500/20`} ${tag.textColor || 'text-white'} rounded-md`}
            >
              {tag.name}
            </span>
          ))}
        ...
        {project.screenshots && project.screenshots.length > 0 && (
          <div className="mb-12">
            ...
            <AdaptiveScreenshotsGallery 
              ...
              projectType={project.primaryColor}
            />
          </div>
        )}
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {relatedProjects.map((relatedProject, index) => (
              <Link
                ...
                className="bg-slate-800 rounded-xl overflow-hidden border border-slate-700 transition-all duration-300 hover:border-slate-600 hover:transform hover:scale-105 hover:shadow-lg"
              >
